 ...
 * Date：15-6-2
 */
public class AssistEntityOperatorImpl<T> implements AssistEntityOperator<T> {

  private static final Logger LOGGER = LoggerFactory.getLogger(AssistEntityOperatorImpl.class);
  ...
  public AssistEntityOperatorImpl(Class<T> tClass) {
    this.tClass = tClass;
  ...
  public FindMsg<T> find(LASQuery query) {
    try {
      ...
      JsonNode jsonNode = ZJsonParser.asJsonNode(response);
      ...
      if (results != null) {
        ...
        for (JsonNode result : results) {
          r.add(ZJsonParser.asObject(result.toString(), tClass));
        }
      }
  ...
  public SaveMsg create(T obj) throws as.leap.code.LASException {
    try {
      String response = WebUtils.doPost(getAPIAddress(), getHeader(), ZJsonParser.asJson(obj), DEFAULT_TIMEOUT, DEFAULT_READ_TIMEOUT);
      LOGGER.info("get response of create[" + getAPIAddress() + "]:" + response);
      return ZJsonParser.asObject(response, SaveMsg.class);
    } catch (Exception e) {
      throw new as.leap.code.LASException(e);
    }
  }
  ...
  public T findById(String id) {
    try {
      ...
      return ZJsonParser.asObject(response, tClass);
    } catch (Exception e) {
      throw new as.leap.code.LASException(e);
    }
  }
  ...
  public DeleteMsg delete(String id) {
    try {
      ...
      return ZJsonParser.asObject(response, DeleteMsg.class);
    } catch (Exception e) {
      throw new as.leap.code.LASException(e);
    }
  }
