 ...
 */
public final class ZEntityManagerHandler<T> implements ZHandler<Request, Response> {

  private static final Logger logger = LoggerFactory.getLogger(ZEntityManagerHandler.class);

  private final ZEntityManager<T> entityManager;
  private final Class<T> entityClazz;
  private final ZEntityManagerHook hook;
  ...
  public ZEntityManagerHandler(ZEntityManager<T> entityManager, ZEntityManagerHook hook, Class<T> entityClazz) {
  ...
  public Response handle(Request request) {
    JsonNode jsonNode = ZJsonParser.asJsonNode(request.parameter(String.class));
    String methodName = jsonNode.get("method").textValue();
    MethodName method = MethodName.getByMethodName(methodName);
    switch (method) {
      case CREATE:
        Response<SaveMsg> createResponse = new ZResponse(SaveMsg.class);
        JsonNode entityJsonObject = jsonNode.get(PARAMS);
        try {
          T entity = ZJsonParser.asObject(entityJsonObject.toString(), entityClazz);
      ...
      case UPDATE:
        Response<UpdateMsg> updateResponse = new ZResponse<UpdateMsg>(UpdateMsg.class);
        JsonNode params = jsonNode.get(PARAMS);
        String objectID = params.get("objectId").textValue();
        JsonNode updateJsonObject = params.get("update");
        try {
          UpdateMsg updateMsg = entityManager.update(objectID, LASUpdate.getUpdate(ZJsonParser.jsonNodeToMap(updateJsonObject)));
      ...
      case DELETE:
        Response<DeleteMsg> deleteResponse = new ZResponse(DeleteMsg.class);
        try {
      ...
      case DELETEBATCH:
        Response<DeleteMsg> deleteBatchResponse = new ZResponse(DeleteMsg.class);
      ...
      case FIND_BY_ID:
        Response findByIdResponse = new ZResponse(entityClazz);
        try {
      ...
      case FIND:
        ...
        if (query != null) {
          sunQuery = new LASQuery(ZJsonParser.jsonNodeToMap(query), keys1);
        } else {
        ...
        Response findResponse = new ZResponse(FindMsg.class);
