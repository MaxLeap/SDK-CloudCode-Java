...
public class CloudCodeBootstrap {
  ...
  void start() {
    ...
    //init data
    final Map<String, JobRunner> jobs = new ConcurrentHashMap<String, JobRunner>();
    final Map<String, Definer> definers = new ConcurrentHashMap<String, Definer>();
    //start metric
    this.metrics.start();
            ...
            public void handle(final HttpServerRequest request) {
              request.bodyHandler(new Handler<Buffer>() {
                ...
                public void handle(Buffer buffer) {
                  executor.execute(new HttpRequest(request, metrics, executor, jobs, definers, buffer.toString("UTF-8")));
                }
              });
            }
          })
